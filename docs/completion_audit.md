...
| Convert at least one score page/excerpt to MIDI | Hrashapar excerpt, full Yegmalian page 55, and mapped `khorurt_khorin` / `hays_hark` sections were processed through Audiveris -> MXL -> MIDI. | Implemented as proof of concept |
...
| Normalize to one organ track | `tools/flatten_to_organ_midi.py` creates one-track organ MIDI; verified `midi/yegmalian_full_page55_wrapper_test_organ.mid`, `midi/khorurt_khorin_organ.mid`, and `midi/hays_hark_organ.mid` have one track. | Implemented |
...
- The full 420-page Yegmalian score has not been batch-transcribed end-to-end; two mapped sections have been batch-exported.
- The generated OMR MIDI has not been manually corrected against the score, although warning checklists now exist in `docs/omr_review_report.md`, `docs/omr_review_report_khorurt_khorin.md`, and `docs/omr_review_report_hays_hark.md`.
