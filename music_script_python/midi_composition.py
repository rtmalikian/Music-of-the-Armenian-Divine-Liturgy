...
from midi_models import VoiceLeadingContext, TensionState, MelodicCell, Motif, BarHarmony
...
def generate_4bar_loop(scale: List[int], root: int, scale_name: str = 'western',
    ...
    persona = choose_melody_persona(is_chorus)
    motif = _generate_seed_motif(scale, root, scale_name, is_chorus)
    ...
    for bar_in_loop in range(4):
        ...
        cell = generate_melodic_cell(
            base=clamp_to_register(active_root + (36 if is_chorus else 24), 'main_melody' if not is_chorus else 'chorus_melody'),
...
def generate_bass(root: int, quality: str, scale: List[int], vc: VoiceLeadingContext,
                  ...
                  harmony: Optional[BarHarmony] = None,
                  next_harmony: Optional[BarHarmony] = None) -> MelodicCell:
    ...
    for i in range(len(rhythm)):
        ...
        elif bar_mod in [1, 3] and random.random() < 0.2: notes.append(None)
        elif i == len(rhythm) - 1 and approach_note is not None and random.random() < 0.55: notes.append(approach_note)
        elif pedal_active and random.random() < 0.65: notes.append(root_note)
        elif i == len(rhythm) - 1 and bar_mod == 3: notes.append(octave)
        elif pattern_style == 'active' and approach_tones and random.random() < 0.25: notes.append(random.choice(approach_tones))
        elif random.random() < 0.3: notes.append(fifth)
...
def generate_harmonic_bass(root: int, quality: str, scale: List[int], vc: VoiceLeadingContext,
                           rhythm: Optional[List[int]] = None, pattern_style: str = 'standard',
                           bar: int = 0, bar_length: int = 1920, time_sig: str = '4-4',
                           harmony: Optional[BarHarmony] = None) -> MelodicCell:
    bar_mod = bar % 4
    if rhythm is None:
        patterns = BASS_PATTERNS.get(time_sig, BASS_PATTERNS['4-4'])
        if bar_mod == 3:
    ...
    notes = []
    for i in range(len(rhythm)):
        if i == 0 or not (len(rhythm) > 2 and i >= 2): notes.append(random.choice(chord_h))
        else:
